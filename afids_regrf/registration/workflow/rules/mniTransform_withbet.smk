rule bet:
    output:
        stripped=bids_name(root=join(config['output_dir'],'derivatives','afids_mni'), subject='{subject}',suffix='stripped.nii.gz',**{'space':'MNI152NLin2009cAsym'}),
...
rule align_mni_rigid:
    input:
        stripped=bids_name(root=join(config['output_dir'],'derivatives','afids_mni'), subject='{subject}',suffix='stripped.nii.gz',**{'space':'MNI152NLin2009cAsym'}),
    output:
        stripped_warped=bids_name(root=join(config['output_dir'],'derivatives','afids_mni'), subject='{subject}',suffix='stripped_aligned.nii.gz',**{'space':'MNI152NLin2009cAsym'}),
        xfm=bids_name(root=join(config['output_dir'],'derivatives','afids_mni'), subject='{subject}',suffix='xfm.mat',**{'space':'MNI152NLin2009cAsym'}),
    params:
...
rule apply_xfm:
    input:
        xfm=bids_name(root=join(config['output_dir'],'derivatives','afids_mni'), subject='{subject}',suffix='xfm.mat',**{'space':'MNI152NLin2009cAsym'}),
    output:
        warped=bids_name(root=join(config['output_dir'],'derivatives','afids_mni'), subject='{subject}',suffix='T1w.nii.gz',**{'space':'MNI152NLin2009cAsym'}),
    params:
        moving = expand(moving_volume, session=sessions, allow_missing=True),
        fixed = config['template'],
    envmodules: 'fsl'
